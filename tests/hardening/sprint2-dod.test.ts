...
describe('Sprint 2 Definition of Done Audit (Tasks 060–063)', () => {
  ...
  // ─────────────────────────────────────────────────────────────────────────
  it(
    '[DoD-S2-01] Task 060: ACP federation/delegation contracts, AgentDirectory, and scope attenuation behave correctly',
    () => {
      ...
      assert.ok(
        SubAgentDelegationRequestSchema,
        'SubAgentDelegationRequestSchema must be exported',
      );
      assert.ok(
        SubAgentDelegationResponseSchema,
      ...
      assert.ok(
        CompositeExecutionReceiptSchema,
        'CompositeExecutionReceiptSchema must be exported',
      );
      ...
      assert.ok(
        envelope.message_id,
        'createFederationMessage must generate a message_id UUID',
      );
      assert.strictEqual(
        envelope.message_type,
        'HEARTBEAT',
        'message_type must round-trip',
      );
      assert.strictEqual(
        envelope.from_agent,
        'agent-a',
        'from_agent must round-trip',
      );
      ...
      const validAttenuation = verifyScopeAttenuation(
  ...
  // ─────────────────────────────────────────────────────────────────────────
  it(
    '[DoD-S2-02] Task 061: Native local-AI runtime, VRAM offloader, hardware detector, and SSRF guard behave correctly',
    async () => {
      // ── 1. Canonical contracts ────────────────────────────────────────────
      ...
      for (const f of expectedFiles) {
        assert.ok(
          fs.existsSync(path.join(localAiDir, f)),
          `${f} must exist in local-ai runtime`,
        );
      }
      ...
      assert.ok(
        ...
        'Execution plan must have a placement object',
      );
      assert.ok(
      ...
      assert.throws(
        ...
        (err: unknown) =>
          err instanceof VramOffloaderError && err.code === 'INVALID_HARDWARE',
      ...
      assert.throws(
        ...
        (err: unknown) =>
          err instanceof ProviderAdapterError && err.code === 'ENDPOINT_DISALLOWED',
  ...
  // ─────────────────────────────────────────────────────────────────────────
  it(
    '[DoD-S2-03] Task 062: SqliteMemoryStore, VectorIndex, and GraphProjectionEngine perform real persistence and tenant isolation',
    async () => {
      // ── 1. Contracts ──────────────────────────────────────────────────────
      assert.ok(
      ...
      assert.ok(
        MemoryGraphQueryRequestSchema,
        'MemoryGraphQueryRequestSchema must be exported',
      );
      assert.ok(
        MemoryGraphQueryResponseSchema,
        'MemoryGraphQueryResponseSchema must be exported',
      );
      ...
      assert.strictEqual(
        created.content,
        record.content,
        'Stored record content must match input',
      );
      ...
      assert.ok(
        retrieved,
        'SqliteMemoryStore.getById must retrieve the previously stored record',
      );
      ...
      assert.strictEqual(
        idx.getDimensions(),
        384,
        'VectorIndex must report correct 384 dimensions',
      );
      ...
      assert.strictEqual(
        node.label,
        'DoD Sprint 2 Test Entity',
        'Node label must round-trip',
      );
  ...
  // ─────────────────────────────────────────────────────────────────────────
  it(
    '[DoD-S2-04] Task 063: Web dashboard Memory Explorer and Knowledge Graph views and DashboardAPIClient methods exist',
    () => {
  ...
  // ─────────────────────────────────────────────────────────────────────────
  it(
    '[DoD-S2-05] Monorepo workspace isolation: packages/contracts and packages/plugin-sdk have no backend/desktop implementation dependencies',
    () => {
      ...
      assert.ok(
        workspaceYaml.includes('services/*'),
        'pnpm-workspace.yaml must include services/*',
      );
      assert.ok(
  ...
  // ─────────────────────────────────────────────────────────────────────────
  it(
    '[DoD-S2-06] Sprint 2 operational runbooks exist for all Sprint 2 failure domains (RB-021 through RB-024)',
    () => {
  ...
  // ─────────────────────────────────────────────────────────────────────────
  it(
    '[DoD-S2-07] docs/RESOURCE_BASELINE.md contains a Sprint 2 measurement section with observed data',
    () => {
  ...
  // ─────────────────────────────────────────────────────────────────────────
  it(
    '[DoD-S2-08] Sprint 2 milestone completion reports exist for Tasks 060 through 063',
    () => {
      ...
      for (const taskNum of [60, 61]) {
        ...
        assert.ok(
          content.length > 500,
          `${reportName} must contain substantive report content`,
        );
      }
  ...
  // ─────────────────────────────────────────────────────────────────────────
  it(
    '[DoD-S2-09] SPRINT_2_COMPLETION_REPORT.md and docs/SPRINT_3_READINESS_AND_BACKLOG.md exist with required sections',
    () => {
      ...
      for (const ref of ['060', '061', '062', '063']) {
        assert.ok(
          s2Report.includes(ref),
          `Sprint 2 completion report must reference Task ${ref}`,
        );
      }
      ...
      assert.ok(
        fs.existsSync(s3ReadinessPath),
        'docs/SPRINT_3_READINESS_AND_BACKLOG.md must exist',
      );
      ...
      assert.ok(
        s3Doc.includes('CANDIDATE') ||
          s3Doc.includes('Candidate') ||
          s3Doc.includes('Sprint 3'),
  ...
  // ─────────────────────────────────────────────────────────────────────────
  it(
    '[DoD-S2-10] Sprint 2 security hardening suites exist for all 4 milestones and are registered in package.json',
    () => {
      ...
      for (const filename of [
        ...
        assert.ok(
          pkgJson.includes(filename),
          `package.json test command must register ${filename}`,
        );
      }
    },
  );
